...
| **~~Chunk loading system~~** ✅ | `scripts/chunks/sk_chunk_manager.gd`, `sk_chunk.gd`, `sk_chunk_utils.gd`, `sk_chunk_source.gd`, `sk_chunk_adapter.gd`, `sk_cancel_token.gd`, `sk_chunk_debug_draw.gd` | Generic engine-agnostic chunk manager with async loading, active/preload radii, mount/unmount lifecycle, LRU cache eviction, concurrency limits, and cancellation. Circular (Euclidean) radius option (`use_circular_radius`). Load retry with configurable `max_load_retries`/`retry_delay`. Multi-origin `update_multi()` for co-op/split-screen. `SKChunkDebugDraw` ImmediateMesh visualizer. Abstract `SKChunkSource`/`SKChunkAdapter` interfaces for game-specific implementations. GUT tests in `tests/test_chunk_manager.gd`. |
...
42. **~~Plugin version bump~~** ✅ — `plugin.cfg` and `skelerealms.gd` updated to `beta 0.9`; `PLUGIN_VERSION` constant added for migration registry and future tooling.
